  ...
  return (
    <button
      className={cn(
        ...
        variant === "ghost" &&
          "text-neutral-700 hover:text-black dark:text-neutral-300 dark:hover:text-white px-4 py-2 text-sm font-medium",
        variant === "icon" &&
          "w-10 h-10 flex items-center justify-center rounded-full hover:bg-black/5 dark:hover:bg-white/10",
        className
      )}
      {...props}
    >
        ...
        <div className="mx-auto px-4 lg:px-10 max-w-7xl">
          <div className="flex items-center justify-between">
            <div className="lg:hidden">
              <Button variant="icon" onClick={() => setMenuOpen((prev) => !prev)} aria-label="Toggle Menu">
            ...
            <div className="flex items-center gap-1">
              <Button variant="icon" onClick={() => setSearchOpen(true)} aria-label="Search">
                <Search className="w-5 h-5 text-black dark:text-white" />
              </Button>

              <Button variant="icon" aria-label="Account" className="hidden sm:flex">
          ...
          {menuOpen && (
            <div className="fixed inset-y-0 left-0 z-50 w-72 bg-white shadow-lg overflow-auto dark:bg-black">
              <div className="flex justify-between items-center p-4 border-b dark:border-neutral-700">
                ...
                <button className="w-8 h-8 flex items-center justify-center" onClick={() => setMenuOpen(false)}>
                  <X className="w-5 h-5" />
                </button>
              </div>
                  ...
                  {navCategories.map((category) => (
                    <a
                      ...
                      className={cn(
                        ...
                        category.highlight ? "text-red-600 font-medium" : "text-neutral-800 dark:text-neutral-300"
                      )}
